...
To test the Cholesky routine for different n and condition number, you can change 'n' and 'cond' in the file settings.json.
...
To test out different 8-bit precisions, you can head into the "pivoted_cholesky.cpp" file and change all instances of "ml_dtypes::float8_ieee_p<p>" to "ml_dtypes::float8_ieee_p<x>", where x is the precision you want to test out with (x - 1 is the number of mantissa bits).

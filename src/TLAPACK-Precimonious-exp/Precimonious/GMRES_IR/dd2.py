...
EXE_COUNT = 1
benchfile = f"{benchmark}.cpp"
TO_RUN = f"../tempscripts/{benchmark}"
TO_RUN_WITH_ARGS = f"../tempscripts/{benchmark} {DIM} 0 {COND} 0 0 5 20 {PRECISION}"
...
def run_orig_config(search_config, original_config, benchmark):
  ...
  if os.path.exists(TO_RUN):
    ...
    process = subprocess.Popen([TO_RUN, str(DIM) ,"0", str(COND),  "0", "0", "5" , "10" , str(PRECISION)])
    try:

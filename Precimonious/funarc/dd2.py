...
def run_config(search_config, original_config, benchmark):
  ...
  if os.path.exists(TO_RUN):
    print("Round 1:")
    #print something here that would be different in this function from the orig_run_config
    print("Running: ", TO_RUN)
    process = subprocess.Popen([TO_RUN, "0"])
    try:
      process.wait(timeout=TIME_OUT)
  ...
  if result == 1:
    ...
    # Q : this file is not created in the original run_config function
    # A : 
    with open('./time.txt', "w") as f:
      f.write(str(avg_runtime))
    runtime = avg_runtime
...
def dd_search_config(change_set, type_set, switch_set, search_config, original_config, benchmark, div, original_score):
  ...
  global glob_min_idx
  print("start of dd_search_config")
  delta_change_set = []
  delta_type_set = []
  delta_switch_set = []
  ...
  for i in range(0, len(delta_change_set)):
    ...
    if len(delta_change) > 0:
      ...
      if run_config(search_config, original_config, benchmark) == 1 and utilities.get_dynamic_score() < original_score:
        ...
        print("made it out of the trenches")
        if score < glob_min_score or glob_min_score == -1:
          print("made it out again")
          pass_inx = i
          inv_is_better = False
          # min_score = score
...
def search_config(change_set, type_set, switch_set, search_config, original_config, benchmark, original_score):
  ...
  print("change set: ", change_set)
  print("type set: ", type_set)
  print("switch set: ", switch_set)
  utilities.to_2nd_highest_precision(change_set, type_set, switch_set)
  print("dyn score is : ", utilities.get_dynamic_score())
  a = run_config(search_config, original_config, benchmark)
  print("a is: ", a)
  if a != 1 or utilities.get_dynamic_score() > original_score:
  ...
  if utilities.get_dynamic_score() < original_score:
    glob_min_score = utilities.get_dynamic_score()

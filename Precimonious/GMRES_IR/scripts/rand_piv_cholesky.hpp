...
int pivoted_cholesky(matrix_t& A, piv_t& left_piv, piv_t& right_piv, chol_mod& chol_modif, n_flops& flop_counter,  int r = 32, double tol = 0.0000000000001, float dropping_prob = 0.1)
{
    ...
    using gemm_type3 = lo_float::float8_ieee_p<5>;
    using gemm_type4 = lo_float::float6_p<4>;
    ...
    for(int i = 0; i < q; i++) 
    {
        ...
        if (can_use_type<gemm_type5>(A11, diag_left ,update_left, tol)) {
            swich = 0;
        ...
        } else if (can_use_type<gemm_type4>(A11, diag_left ,update_left, tol)) {
            swich = 2;
            cout << " using lowest precision \n"; //8 bit float fp8
            std::vector<gemm_type3> buf_L_(r * (n - (i+1)*r));
            auto buf_L = gemm_3_matrix(buf_L_, n - (i+1)*r, r);
        ...
        } else if (can_use_type<gemm_type3>(A11, diag_left ,update_left, tol)) {
            swich = 3;
            cout << " using lowest precision \n"; //8 bit float fp8
            std::vector<gemm_type3> buf_L_(r * (n - (i+1)*r));
            auto buf_L = gemm_3_matrix(buf_L_, n - (i+1)*r, r);

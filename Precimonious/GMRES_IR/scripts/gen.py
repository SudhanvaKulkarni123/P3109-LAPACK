...
def main():
    #if dir test_matrices doesn't exist, create it
    num = int(sys.argv[1])
    if not os.path.exists("matrix_collection_" + str(num)):
        os.makedirs("matrix_collection_" + str(num))
    n = int(sys.argv[2])
    cond = int(sys.argv[3])
    space = cond/5
    if cond/5 < n: 
        space = int(cond/5)
    matrices = LU_gen(n, cond, space, False, num)
    true_cond = int(matrices[-1])
    filename = f"matrix_collection_{num}/matrix_{true_cond}.csv"
    count = 0

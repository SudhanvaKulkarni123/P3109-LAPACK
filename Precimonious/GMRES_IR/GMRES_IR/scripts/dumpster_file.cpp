...
reporter GMRES_IR(size_t n, ml_dtypes::float8_ieee_p<4> scale, float cond, int p,  int variant = 0, int num_iter_1 = 0, int num_total_iter = 5) 
{   
    ...
    double total_time = 0.0; //store weighted opcounts in this variable
    ...
    int info, infotoo;
    if(variant == 0)   {info = tlapack::getrf(LU, piv_lo, tlapack::GetrfOpts{GetrfVariant::Recursive}); total_time += get_adjusted_opcount("getrf", LU, LU, b, b);}
    else {info = tlapack::getrf(LU, piv_lo, tlapack::GetrfOpts{GetrfVariant::Level0}); total_time += get_adjusted_opcount("getrf", LU, LU, b, b);}
    if (info != 0) {
    ...
    total_time += get_adjusted_opcount("trsv", LU_copy_C, LU_copy_C, b_3, b_3);
    ...
    while(count < num_iter_1) { 
        ...
        total_time += get_adjusted_opcount("gemv", FG, FG, x, r);
        ...
        total_time += get_adjusted_opcount("trsv", LU_copy_B, LU_copy_B, solved_r_2, solved_r_2);
        ...
        while(num_iter < 15 && inner_res_norm > tol*static_cast<double>(normb_B)) {
            ...
            total_time += get_adjusted_opcount("Arnoldi_iter", A_B, H_B, Q_B, solved_r_2);
            num_iter = num_iter + 1; 
            for(int i = 0; i < n; i++) {
                for(int j = 0; j < n; j++) {
                    H_copy_B(i,j) = 0.0;
            ...
            if(num_iter != n){ Hessenberg_qr(tlapack::slice(H_copy_B,range{0, num_iter+1}, range{0,num_iter}), tlapack::slice(be_1_2,range{0,  num_iter+1}), n); total_time += get_adjusted_opcount("Hessenberg_qr", H_copy_B, be_1_2, n);}
            else  {Hessenberg_qr(H_copy_B, be_1_2, n); total_time += get_adjusted_opcount("Hessenberg_qr", H_copy_B, be_1_2, n);}
            
            auto da_tmp = tlapack::slice(be_1_2,range{0, num_iter});
            if(num_iter != n) {tlapack::trsv(Uplo::Upper, tlapack::NO_TRANS,tlapack::Diag::NonUnit ,tlapack::slice(H_copy_B,range{0, num_iter}, range{0,num_iter}), da_tmp); total_time += get_adjusted_opcount("trsv", H_copy_B, H_copy_B, da_tmp, da_tmp);}
            else {tlapack::trsv(Uplo::Upper, tlapack::NO_TRANS, tlapack::Diag::NonUnit, H_copy_B, da_tmp); total_time += get_adjusted_opcount("trsv", H_copy_B, H_copy_B, da_tmp, da_tmp);}
            //our solution vector is now obtained by multiplying by Q_n
            if(num_iter != n) {tlapack::gemv(tlapack::NO_TRANS, static_cast<T2>(1.0), tlapack::slice(Q_B,range{0, n}, range{0,num_iter}), tlapack::slice(be_1_2, range{0, num_iter}), static_cast<T2>(0.0), solved_r_2); total_time += get_adjusted_opcount("gemv", Q_B, be_1_2, solved_r_2);}
            else {tlapack::gemv(tlapack::NO_TRANS, static_cast<T2>(1.0), Q_B, be_1_2, static_cast<T2>(0.0), solved_r_2); total_time += get_adjusted_opcount("gemv", Q_B, be_1_2, solved_r_2);}
        }
        //update r
        for(int i = 0; i < n; i++) solved_r[i] = static_cast<double>(solved_r_2[i]);
        tlapack::axpy(1.0, solved_r, x);
        total_time += get_adjusted_opcount("axpy", solved_r, x, x, x);
    ...
    while(count < num_total_iter) {
        ...
        total_time += get_adjusted_opcount("trsv", LU_copy, LU_copy, solved_r_f, solved_r_f);
        ...
        while(num_iter < 10 && inner_res_norm > tol*normb) {
            ...
            total_time += get_adjusted_opcount("Arnoldi_iter", A_float, H, Q, solved_r_f);
            num_iter = num_iter + 1; 
            for(int i = 0; i < n; i++) {
                for(int j = 0; j < n; j++) {
                    H_copy(i,j) = 0.0;
            ...
            if(num_iter != n) {Hessenberg_qr(tlapack::slice(H_copy,range{0, num_iter+1}, range{0,num_iter}), tlapack::slice(be_1_f,range{0,  num_iter+1}), n); total_time += get_adjusted_opcount("Hessenberg_qr", H_copy, be_1_f, n);}
            else  { Hessenberg_qr(H_copy, be_1_f, n); total_time += get_adjusted_opcount("Hessenberg_qr", H_copy, be_1_f, n);}
            
            auto da_tmp = tlapack::slice(be_1_f,range{0, num_iter});
            if(num_iter != n) {tlapack::trsv(Uplo::Upper, tlapack::NO_TRANS,tlapack::Diag::NonUnit ,tlapack::slice(H_copy,range{0, num_iter}, range{0,num_iter}), da_tmp); total_time += get_adjusted_opcount("trsv", H_copy, H_copy, da_tmp, da_tmp);}
            else {tlapack::trsv(Uplo::Upper, tlapack::NO_TRANS, tlapack::Diag::NonUnit, H_copy, da_tmp); total_time += get_adjusted_opcount("trsv", H_copy, H_copy, da_tmp, da_tmp);}
            //our solution vector is now obtained by multiplying by Q_n
            if(num_iter != n) {tlapack::gemv(tlapack::NO_TRANS, 1.0, tlapack::slice(Q,range{0, n}, range{0,num_iter}), tlapack::slice(be_1_f, range{0, num_iter}), 0.0, solved_r_f); total_time += get_adjusted_opcount("gemv", Q, be_1_f, solved_r_f);}
            else {tlapack::gemv(tlapack::NO_TRANS, 1.0, Q, be_1_f, 0.0, solved_r_f);  total_time += get_adjusted_opcount("gemv", Q, be_1_f, solved_r_f);}
        }
        //update r
    ...
    reporter to_ret = reporter{0.0, 0.0};
    to_ret.error = res_norm;
    to_ret.time = total_time;
...
int main(int argc, char** argv) {
    ...
    er3 += GMRES_IR<bfloat16, float>(n, ml_dtypes::float8_internal::numeric_limits_float8_ieee_p<4>::max()/floate4m3{2.0}, static_cast<float>(atoi(argv[3])), atoi(argv[8]), atoi(argv[4]), num_iter_1, total_num_iter);    
    
    bool verified = er3 < 1e-6;
